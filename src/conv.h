...
#define RandomNumber(Min, Max) (Min + (Random() % (Max - Min + 1)))
    ...
    _FORCE_INLINE constexpr char obfuscateChar(const char Character, unsigned Index) {
    ...
    public:
        ...
        static void regcall deobfuscateFunc(char * Value,uintptr_t i,unsigned char key) {
          Value[i] = 0;
          do{
            i--;
        ...
        static void regcall deobfuscateWFunc(char * Value,uintptr_t i,unsigned char key) {
          *(wchar_t *)((wchar_t*)Value+i) = 0;
          do{
            i--;
            *(wchar_t *)((wchar_t*)Value+i) = (unsigned char)(*(unsigned char *)(Value+i) - (key + i));
          }while(i);
        }
        char* get() {
          return Value;
...
#undef _T
  #define _C( x ) ( str_obfuscate::obfuscate<str_obfuscate::ConstructIndexList<sizeof( x ) - 1>::Result>( x ).deobfuscate() )
#ifdef _UNICODE
#define _T( x ) ( str_obfuscate::obfuscate<str_obfuscate::ConstructIndexList<sizeof( x ) - 1>::Result>( x ).deobfuscateW() )
#else
  #define _T( x ) _C( x )
#endif
#endif
...
wchar_t *regcall _conv2ws(char *s, wchar_t *m,size_t sz); //convert chars to wide chars
char *regcall _conv2a(wchar_t *s, char *m); //convert wide string to string
char *regcall _conv2as(wchar_t *s, char *m,size_t sz); //convert wide chars to chars
...
unsigned regcall hash_rtasi(char * str,uintptr_t sz); //hash chars case-insensitive
...
unsigned regcall hash_rtwsi(wchar_t * str,uintptr_t sz); //hash wide chars case-insensitive
#ifdef __cplusplus
}
#endif
